(() => {
  ...
  const IS_DESKTOP_RUNTIME = Boolean(window.__TAURI__ || window.__TAURI_INTERNALS__ || navigator.userAgent.includes('Tauri'));
  let notesAutoSaveTimer = null;
  let notesDragState = null;
  let attendanceChronoTimer = null;
  let expenseAutoSaveTimers = {};
  let aiBriefActiveTab = 'project';
...
- Estrategia de actualizacion: SIN updater automatico.
- Politica activa: cuando haya cambios mayores, se genera nueva build, se desinstala version anterior y se instala la nueva.
- Resguardo de datos: se conserva por exportacion/importacion de bases desde Central de respaldos.
- Estado recuperado post-incidente: modulos y flujos criticos revalidados (Asistencia, Panel OT, respaldos, visor PDF interno, descargas Desktop).
...
- Frontend estatico: index.html + assets/js + assets/css.
- Estado persistente en localStorage via ERMStorage.
- Estado base en ERMDefaults (data.js).
- Calculos centrales en calculations.js.
- UI/eventos en app.js.
- Desktop con Tauri 2.x (sin plugin updater), con comandos nativos para abrir enlaces y guardar archivos (PDF y respaldos) con flujo robusto.
...
- Respaldos: en Desktop se usa dialogo nativo Guardar como; en navegador se usa File System Access API o descarga estandar.
- PDFs: visor interno embebido y descarga robusta en Desktop mediante comando nativo.
...
- Validar errores de archivos editados y compilacion desktop antes de cerrar.
...
- Entender state global y ramas: scenario, quote, database, inventory, expenses, contacts, orders, ui.
- Entender tambien la rama attendance y los timestamps de respaldo por base.
- Respetar lenguaje visual actual y no introducir cambios de UX no solicitados.
- Si se toca flujo de release, mantener politica sin updater automatico salvo instruccion explicita del usuario.
...
- Consolidar AndiApp como sistema interno confiable para decidir precios, ejecutar OT con control y conservar historial operacional util para crecer con orden.`;
  ...
  const AI_RELEASE_PLAYBOOK_TEXT = `AndiApp · Playbook de release no-estable (sin updater automatico)

Politica oficial de actualizacion
- No usar actualizacion dentro de la app.
- Para cambios mayores: crear nueva build, desinstalar version anterior e instalar la nueva.
- Respaldar y restaurar datos mediante export/import de bases.

Checklist previo a build final
1) Definir alcance del release (features/fixes) y riesgos.
2) Validar errores en archivos tocados.
3) Validar flujos criticos: Escenario, Presupuestador, Base de datos, Clientes, Panel OT, Inventario, Gastos, Asistencia y Central de respaldos.
4) Verificar modo dark y modo escritorio en modulos sensibles (tablas y panel OT).
5) Verificar Desktop Save As en exportaciones (JSON/ZIP/PDF) y fallback en navegador.

Checklist de respaldo de datos
1) Exportar bases importantes (individuales o global).
2) Verificar que respaldos contienen datos clave:
   - imageDataUrl e info de imagen.
   - pdfDataUrl e info de PDF.
  - attendance.activeSession y attendance.records cuando corresponda.
3) Probar importacion de respaldo para confirmar integridad.

Versionado recomendado
1) Alinear version en package.json, src-tauri/tauri.conf.json y src-tauri/Cargo.toml.
2) Mostrar version visible en footer como release no-estable.

Build desktop
- Build final general:
  npm run tauri:build

Instalacion limpia de la nueva version
1) Cerrar AndiApp.
2) Respaldar datos desde Central de respaldos.
3) Desinstalar/eliminar app anterior.
4) Instalar app nueva desde DMG.
5) Abrir y validar version visible.
6) Importar respaldos si corresponde.

Cierre de release
1) Registrar cambios y riesgos en base IA del proyecto.
2) Guardar evidencia de build (comando + salida clave + artefactos).
3) Dejar listados artefactos finales con ruta completa.
4) Confirmar que dist y fuente quedaron sincronizados tras el build web.
...
- Proximo paso:`;
  ...
  async function storeAttachmentFromFile(file, category = '') {
    ...
    if (IS_DESKTOP_RUNTIME && typeof invoke === 'function') {
      const dataBase64 = await readFileAsBase64(file);
      const stored = await invoke('store_attachment_file', {
        dataBase64,
        fileName: String(file.name || 'documento.bin'),
        mimeType: safeMime
      });
      return createAttachmentRecord({
        id: stored?.id || uid('att'),
        fileName: stored?.fileName || file.name || 'documento.bin',
        mimeType: stored?.mimeType || safeMime,
        sizeBytes: Number(stored?.sizeBytes || file.size || 0) || 0,
  ...
  async function resolveAttachmentDataUrl(attachment) {
    ...
    const invoke = getTauriInvoke();
    if (!IS_DESKTOP_RUNTIME || typeof invoke !== 'function') return '';
    try {
      const dataUrl = await invoke('read_attachment_as_data_url', {
        storagePath: attachment.storagePath,
        mimeType: attachment.mimeType || 'application/octet-stream'
      });
      return String(dataUrl || '');
    } catch (error) {
      console.error('No se pudo leer el adjunto interno.', error);
      return '';
    }
  }

  async function removeInternalAttachmentFile(attachment) {
    if (!attachment?.storagePath || !IS_DESKTOP_RUNTIME) return;
    const invoke = getTauriInvoke();
    if (typeof invoke !== 'function') return;
    try {
      await invoke('delete_attachment_file', { storagePath: attachment.storagePath });
    } catch (error) {
      console.warn('No se pudo eliminar el archivo adjunto interno.', error);
    }
  }
...
<body>
  ...
  function confirmSafe(message) {
    try {
      // In desktop WebView, native confirm can fail silently and always block actions.
  ...
  async function savePdfToDesktopDownloads(dataUrl, fileName) {
    if (!IS_DESKTOP_RUNTIME) return false;
    const invoke = getTauriInvoke();
    if (typeof invoke !== 'function') return false;
    const payload = String(dataUrl || '').trim();
    if (!payload.startsWith('data:application/pdf;base64,')) return false;

    try {
      const savedPath = await invoke('save_pdf_to_downloads', {
        dataUrl: payload,
        fileName: String(fileName || 'documento.pdf')
      });
      if (savedPath) {
        window.alert(`PDF guardado en:\n${savedPath}`);
      }
      return true;
    } catch (error) {
      console.warn('No se pudo guardar el PDF con el comando de escritorio.', error);
      return false;
    }
  }
  ...
  function blobToBase64(blob) {
    return new Promise((resolve, reject) => {
      ...
      reader.onerror = () => reject(new Error('No se pudo convertir el archivo para guardado de escritorio.'));
      reader.readAsDataURL(blob);
    });
  }

  async function saveBlobWithDesktopDialog(blob, fileName) {
    if (!IS_DESKTOP_RUNTIME) return false;
    const invoke = getTauriInvoke();
    if (typeof invoke !== 'function') return false;
    ...
    try {
      ...
      const savedPath = await invoke('save_file_with_dialog', {
        dataBase64,
        fileName: String(fileName || 'archivo')
      });
      if (savedPath) {
        window.alert(`Archivo guardado en:\n${savedPath}`);
      }
      return true;
    } catch (error) {
      ...
      console.warn('No se pudo abrir Guardar como en escritorio. Se usará descarga estándar.', error);
      return false;
    }
  }
  ...
  async function openExternalLink(url) {
    ...
    if (typeof tauriInvoke === 'function') {
      try {
        ...
        console.error('No se pudo abrir enlace externo con Tauri.', error);
      }
    }
  ...
  function init() {
    ...
    refs.footerRuntimeMode = document.getElementById('footer-runtime-mode');
    ...
    refs.aiBriefTitle = document.getElementById('ai-brief-title');
    refs.aiBriefSubtitle = document.getElementById('ai-brief-subtitle');
    refs.aiBriefTabProject = document.getElementById('ai-brief-tab-project');
    refs.aiBriefTabPlaybook = document.getElementById('ai-brief-tab-playbook');
  ...
  async function migrateLegacyAttachmentsToUnified() {
    ...
    const migrateLegacyAttachment = async (legacy) => {
      if (!legacy?.dataUrl) return null;
      const attachment = createAttachmentRecord(legacy);
      if (!IS_DESKTOP_RUNTIME) return attachment;
      const invoke = getTauriInvoke();
      if (typeof invoke !== 'function') return attachment;
      try {
        const payload = String(legacy.dataUrl || '');
        const base64 = payload.includes(',') ? payload.split(',')[1] : '';
        ...
        return createAttachmentRecord({
          ...
          sizeBytes: Number(stored?.sizeBytes || attachment.sizeBytes || 0) || 0,
          storagePath: String(stored?.storagePath || ''),
          dataUrl: '',
        ...
        console.error('No se pudo migrar adjunto legacy al almacenamiento interno.', error);
        return attachment;
      }
    };
  ...
  function bindEvents() {
    ...
    document.addEventListener('click', async (event) => {
      ...
      if (action === 'select-ai-brief-tab') {
        setAiBriefTab(actionBtn.dataset.tab || 'project');
        return;
      }
      ...
      if (action === 'clear-expense-pdf') {
        ...
        if (targetEntry) {
          ...
          if (currentAttachment) {
            await removeInternalAttachmentFile(currentAttachment);
          }
        }
      ...
      if (action === 'download-expense-pdf') {
        ...
        if (await savePdfToDesktopDownloads(attachmentDataUrl, attachment.fileName || `comprobante-${entryId}.pdf`)) return;
      ...
      if (action === 'download-herramienta-pdf') {
        ...
        if (await savePdfToDesktopDownloads(attachmentDataUrl, attachment.fileName || 'comprobante.pdf')) return;
        const blob = dataUrlToBlobSafe(attachmentDataUrl);
      ...
      if (action === 'download-order-invoice-pdf') {
        ...
        if (await savePdfToDesktopDownloads(attachmentDataUrl, attachment.fileName || `${order.orderNumber || 'ot'}-boleta.pdf`)) return;
        const blob = dataUrlToBlobSafe(attachmentDataUrl);
      ...
      if (action === 'download-inline-pdf-viewer') {
        ...
        if (await savePdfToDesktopDownloads(viewer.dataUrl, viewer.fileName || 'documento.pdf')) return;
      ...
      if (action === 'clear-order-invoice-pdf') {
        ...
        await removeInternalAttachmentFile(attachment);
    ...
    document.addEventListener('change', async (event) => {
      ...
      try {
        ...
        if (previousAttachment) {
          await removeInternalAttachmentFile(previousAttachment);
        }
  ...
  async function copyAiBriefToClipboard() {
    const activeText = aiBriefActiveTab === 'playbook' ? AI_RELEASE_PLAYBOOK_TEXT : AI_PROJECT_CONTEXT_TEXT;
    const activeLabel = aiBriefActiveTab === 'playbook' ? 'Playbook de actualizaciones' : 'Base IA del proyecto';
    try {
      await navigator.clipboard.writeText(activeText);
      window.alert(`${activeLabel} copiado.`);
    } catch (error) {
      window.alert('No se pudo copiar automáticamente. Puedes copiarlo manualmente desde el cuadro.');
    }
  }

  function setAiBriefTab(tab) {
    aiBriefActiveTab = tab === 'playbook' ? 'playbook' : 'project';
    const isPlaybook = aiBriefActiveTab === 'playbook';

    if (refs.aiBriefTabProject) {
      refs.aiBriefTabProject.classList.toggle('active', !isPlaybook);
      refs.aiBriefTabProject.setAttribute('aria-selected', String(!isPlaybook));
    }
    if (refs.aiBriefTabPlaybook) {
      refs.aiBriefTabPlaybook.classList.toggle('active', isPlaybook);
      refs.aiBriefTabPlaybook.setAttribute('aria-selected', String(isPlaybook));
    }
    if (refs.aiBriefTitle) {
      refs.aiBriefTitle.textContent = isPlaybook ? 'Playbook de actualizaciones' : 'Base IA del proyecto';
    }
    if (refs.aiBriefSubtitle) {
      refs.aiBriefSubtitle.textContent = isPlaybook
        ? 'Guia operativa para construir, validar y publicar releases sin perdida de informacion.'
        : 'Documento maestro para transferir contexto del proyecto a otra IA.';
    }
    if (refs.aiBriefContent) {
      refs.aiBriefContent.value = isPlaybook ? AI_RELEASE_PLAYBOOK_TEXT : AI_PROJECT_CONTEXT_TEXT;
  ...
  function renderFooterMeta() {
    ...
    if (refs.footerRuntimeMode) {
      refs.footerRuntimeMode.textContent = IS_DESKTOP_RUNTIME ? 'Modo escritorio' : 'Modo navegador';
    }
  ...
  async function downloadBlobFile(blob, filename) {
    ...
    if (await saveBlobWithDesktopDialog(blob, safeName)) {
      return;
    }
